...
import { diffText, DEFAULT_CONTEXT_LINES, DEFAULT_MAX_DIFF_CELLS } from './diff.ts'
import type {
  ChangeSetSummary, FileChangeSummary, StoredChangeSet, StoredFileChange, TurnSummary,
} from './types.ts'
...
export class ChangeSetStore {
   ...
   */
  async append(record: StoredChangeSet): Promise<void> {
   ...
   */
  async loadTurns(sessionId: SessionId): Promise<StoredChangeSet[]> {
    try {
  ...
  private async loadRaw(sessionId: SessionId): Promise<StoredChangeSet[]> {
    ...
    const records: StoredChangeSet[] = []
    for (let index = candidates.length - 1; index >= 0; index -= 1) {
      const record = candidates[index]
      if (record === undefined || seen.has(record.turn)) continue
   ...
   */
  async latest(sessionId: SessionId): Promise<StoredChangeSet | undefined> {
  ...
  async listSessions(): Promise<SessionId[]> {
    try {
      ...
      return names
        ...
        .map(name => name.slice(0, -'.jsonl'.length) as SessionId)
    } catch {
      return []
    }
  }
...
function parseRecords(content: string | undefined): StoredChangeSet[] {
  if (content === undefined || content === '') return []
  const records: StoredChangeSet[] = []
  for (const line of content.split('\n')) {
    if (line === '') continue
    try {
      records.push(JSON.parse(line) as StoredChangeSet)
    } catch {
 ...
 */
async function commitFile(temporary: string, target: string): Promise<void> {
...
export function summarizeChangeSet(record: StoredChangeSet): ChangeSetSummary {
  const files: FileChangeSummary[] = record.files.map(file => ({
    path: file.path,
    status: file.status,
    kind: file.kind,
...
export function summarizeTurn(record: StoredChangeSet): TurnSummary {
  return {
    turn: record.turn,
    startedAt: record.startedAt,
    finishedAt: record.finishedAt,
 ...
 */
export function mergeSessionChangeSets(turns: readonly StoredChangeSet[]): ChangeSetSummary | null {
  if (turns.length === 0) return null
  // path -> { baseline: string | null (null = absent at session start), final: string | null }
  const states = new Map<string, { baseline: string | null; final: string | null }>()
  ...
  for (const [path, state] of states) {
    ...
    if (before === after) continue // Net-zero across the session.
...
export function storedFileOf(record: StoredChangeSet, path: string): StoredFileChange | undefined {
